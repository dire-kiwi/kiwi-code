...
export function ClaudeNativePane({
      ...
      case 'claude_native_ready': {
        ...
        if ((prompt || imagePaths.length > 0) && !initialPromptSentRef.current) {
          ...
          appendActivity(
            'prompt',
            imagePaths.length > 0
              ? `Initial prompt sent to Claude with ${imagePaths.length} attachment${imagePaths.length === 1 ? '' : 's'}.`
  ...
  async function sendPrompt() {
    ...
    try {
      ...
      appendActivity(
        'prompt',
        `Prompt sent to Claude${uploads.length > 0 ? ` with ${uploads.length} attachment${uploads.length === 1 ? '' : 's'}` : ''}.`,
        sentAt,
      )
      clearSubmittedDraft()
      setNotice('')
