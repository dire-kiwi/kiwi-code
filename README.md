...
Any retained script can be saved as `/<name>` to the closest project `.claude/workflows/<name>.js` or the personal `$CLAUDE_CONFIG_DIR/workflows/` (default `~/.claude/workflows/`) directory. Project commands override personal commands, and the closest project definition wins in a monorepo. Claude Code discovers those files natively; Pi registers the same files as slash commands and can also run them with structured `args`. Direct `create_child_threads`-style tools and the general child-creation endpoint remain disabled so general-purpose orchestration is owned by this server-side workflow control plane. Global, project, and per-thread nesting and usage limits apply to every workflow and child.
