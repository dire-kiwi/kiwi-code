...
The Settings page can install the bundled `dire-mux-processes` and `dire-mux-threads` Agent Skills into `~/.agents/skills/`. Their dependency-free `.mjs` helpers let agents manage persistent processes; list, create, rename, archive, restore, and close threads; and read bounded tmux output from Pi, Claude Code, shell, tool, and process panes. New Pi sessions discover them automatically; an existing Pi session can load them with `/reload`. Claude Code launched through Dire Mux receives the process and workflow skills directly from the bundled Dire Mux plugin, so it does not depend on the global skill installation. New Claude sessions load plugin updates automatically; an existing session can use `/reload-plugins`.
...
Dire Mux loads bundled Pi extensions into Pi sessions and a bundled integration plugin into Claude Code sessions. Claude Code (with gpt) points the same `claude` executable at CLIProxyAPI, maps its Opus, Sonnet, Haiku, and small-model defaults to the selected GPT model, and stores all Claude configuration and session state under `<data-dir>/claude-code-gpt-profile` instead of the user's normal Claude profile. It also explicitly loads the existing sandbox plugin from the normal profile, unsets inherited Anthropic and Claude OAuth credentials, and supplies only CLIProxyAPI's client token, so this profile does not sign in to Claude. The Claude plugin includes lifecycle and thread-title hooks, process and workflow skills, an MCP workflow control server, and an MCP server that exposes the thread's in-app `browser_*` tools; Claude Code's built-in tool search progressively loads those MCP definitions. Pi, Pi Native, and both Claude Code modes in the same thread intentionally share the same in-app browser session. Dire Mux launches Claude Code with `--dangerously-skip-permissions` and suppresses Claude's bypass-mode warning for that session; the separately installed sandbox-exec plugin still runs its `PreToolUse` hooks and remains the permission boundary. The Pi command line receives the bundled thread-title, activity, context, workflow, forked-skill, and child-thread compatibility extension stack. Child relationship metadata is not passed to Claude Code, direct child-thread tools remain disabled, and workflow workers remain Pi Native even when either Claude mode starts the workflow. After the first user message, Pi and both Claude Code modes use a short isolated Pi call with `openai-codex/gpt-5.6-luna` and low reasoning to generate a concise thread title. They update through the Dire Mux API and, for a worktree thread, rename its managed branch to `dire-mux/<title-slug>-<thread-id-prefix>`. Title generation uses Pi's normal authentication.

The Pi extension and Claude Code plugin both report agent lifecycle to Dire Mux. A spinner beside a thread means either agent is working; when all active work settles, it becomes a blue unread indicator. Opening that thread clears the indicator. If the thread was already open when an agent finished, the next interaction inside its workspace clears it.
...
- `POST /api/projects/{id}/threads/{threadId}/children` remains disabled for direct managed-agent calls and returns `503 Service Unavailable`. The server-owned workflow agent route reuses this child transaction after validating a scoped workflow capability; workers remain Pi Native.
...
- `GET /api/projects/{id}/threads/{threadId}/events` streams named `thread-status` snapshots for Pi context status, Git branches, process shells, Shell tmux windows, and retained workflow progress. Dire Mux mutations wake the stream immediately, a shared tmux control-mode client reports window and command changes, and repository reconciliation detects Git changes made directly in a terminal without browser polling.
