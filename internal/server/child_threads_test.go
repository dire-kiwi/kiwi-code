...
func TestChildRollbackDefersArtifactsAfterNativeRemoveFailureAndRecoversOnRestart(t *testing.T) {
	...
	native := newPiNativeManager(store.DataDirectory(), nil, nil, nil, "token", "")
	...
	restarted := &Server{
		...
		terminal:    &terminalHandler{nativePi: newPiNativeManager(restartedStore.DataDirectory(), nil, nil, nil, "token", "")},
