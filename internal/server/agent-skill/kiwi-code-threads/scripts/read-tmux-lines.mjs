...
const supportedTargets = new Set(["pi", "codex", "grok", "claude", "claude-gpt", "terminal", "shell", "nvim", "lazygit"]);
...
run(async () => {
  ...
  if (!target || args.length > 2 || !Number.isInteger(lines) || lines < 1 || lines > 5000) {
    usage("Usage: read-tmux-lines.mjs <pi|codex|grok|claude|claude-gpt|terminal|nvim|lazygit|process:ID> [lines: 1-5000] [--thread <thread-id>] [--project <project-id>] [--window <shell-index>]");
    return;
  }
  ...
  if (target.startsWith("process:")) {
    ...
    if (target === "pi" || target === "codex" || target === "grok" || target === "claude" || target === "claude-gpt") {
      query.set("tool", "pi");
