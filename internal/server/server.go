...
func NewWithOptions(projects *project.Store, options Options) (http.Handler, error) {
	...
	mux.HandleFunc("PATCH /api/projects/{id}/threads/{threadId}/workspace", server.updateThreadWorkspace)
...
func (s *Server) updateThread(w http.ResponseWriter, r *http.Request) {
	...
	fieldCount := 0
	for _, provided := range []bool{input.Title != nil, input.TitleLocked != nil, input.Settled != nil} {
		if provided {
			fieldCount++
		}
	}
	...
	case input.TitleLocked != nil:
		thread, err = s.projects.SetThreadTitleLocked(
			r.PathValue("id"),
			r.PathValue("threadId"),
			*input.TitleLocked,
		)
	case input.Settled != nil:
		thread, err = s.setThreadSettlement(r.PathValue("id"), r.PathValue("threadId"), *input.Settled, time.Now().UTC())
	...
	if errors.Is(err, errThreadWorking) || errors.Is(err, errTerminalStopping) {
	...
	if errors.Is(err, project.ErrThreadTitleLocked) {
		writeError(w, http.StatusConflict, "The thread title is locked.")
		return
	}
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
...
func (s *Server) deleteThread(w http.ResponseWriter, r *http.Request) {
	if failure := s.deleteThreadRecord(r.PathValue("id"), r.PathValue("threadId")); failure != nil {
...
func (s *Server) deleteThreadRecord(projectID, threadID string) *deleteThreadFailure {
