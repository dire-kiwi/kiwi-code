...
description: Controls the browser owned by the current Kiwi Code thread with browser_* MCP tools. Use whenever a task requires opening a URL, inspecting or testing a rendered web page, clicking or filling UI, taking screenshots, evaluating page JavaScript, or managing tabs.
...
1. Navigate to the target, or inspect and select an existing tab.
2. Use `browser_snapshot` before interacting. Prefer its refs over selectors.
3. Take a fresh snapshot after navigation or substantial DOM replacement before reusing refs.
4. Use condition-based `browser_wait` calls instead of arbitrary sleeps where possible.
5. Use screenshots when visual layout matters; use evaluate or raw CDP only when focused tools are insufficient.
