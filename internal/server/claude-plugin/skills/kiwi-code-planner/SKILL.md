...
description: Plans implementation work in a read-only forked child and publishes the resulting Markdown plan to the parent Kiwi Code thread. Use only to create or revise a plan. Do not invoke it to execute a saved plan; load the retained plan and carry it out in the parent context instead.
compatibility: Requires a Kiwi Code-managed Claude Code session with the bundled plan MCP tools.
context: fork
agent: Plan
metadata:
  author: kiwi-code
...
Claude Code may defer MCP tools until needed. If `publish_thread_plan` is not loaded, use Claude Code's built-in `ToolSearch` to find that exact tool.
...
The backend associates the upload with the current Kiwi Code parent thread. If publishing fails, report the failure and do not claim that the plan was saved.
