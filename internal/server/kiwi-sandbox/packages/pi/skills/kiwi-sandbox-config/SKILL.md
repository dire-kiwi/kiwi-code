---
name: kiwi-sandbox-config
description: Inspect, create, and edit Kiwi Sandbox global or project configuration files. Use when the user asks to configure sandbox paths, filesystem permissions, network access, shells, or command rules.
---

# Kiwi Sandbox configuration

Use this skill when the user asks where Kiwi Sandbox is configured, how its JSON file is structured, or asks you to inspect, create, or edit its policy.

## Editing workflow

1. Confirm whether the change belongs in the global or project configuration. Prefer the project file for project-specific access.
2. Read the existing file when present. If it is absent, begin with `{}` for a project overlay or the complete example below for a global policy.
3. Preserve unrelated top-level fields and command rules.
4. Make the narrowest requested policy change. Warn before adding a command rule without `files`, because that grants unrestricted filesystem access.
5. Keep the file valid JSON without comments, then read it back and verify its structure.
6. Kiwi Sandbox protects both active config files from writes while enforcement is enabled. Ask the user to run `/kiwi-sandbox-disable`, make the confirmed edit, and tell them to immediately run `/kiwi-sandbox-enable` afterward. Never silently weaken policy to make a configuration edit succeed.
...
When helping modify a configuration, preserve unrelated fields, keep unrestricted patterns narrow, and clearly warn before adding a command rule that omits `files`.
