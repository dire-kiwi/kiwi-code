...
func TestClaudePluginNamesThreadWithPiFromFirstPrompt(t *testing.T) {
	...
	// Globally installed extensions can register the configured model's
	// provider, so the hook must not disable them.
	if strings.Contains(string(piArgs), "--no-extensions") {
		t.Fatalf("Pi title arguments %q disable extensions", piArgs)
	}
	mu.Lock()
