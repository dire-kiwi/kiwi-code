...
func withProjectMutationResult[T any](s *Store, operation func() (T, error)) (result T, err error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) withProjectMutation(operation func() error) error {
	_, err := withProjectMutationResult(s, func() (struct{}, error) {
		return struct{}{}, operation()
	})
	return err
}

func saveProjectMutationResult[T any](s *Store, result T, rollback func()) (T, error) {
...
func (s *Store) saveProjectMutation(rollback func()) error {
	_, err := saveProjectMutationResult(s, struct{}{}, rollback)
	return err
}
...
func (s *Store) RecordThreadPrompt(projectID, threadID string, promptedAt time.Time) (Thread, error) {
...
func (s *Store) UpdateThreadTitle(projectID, threadID, title string, autoGenerated bool) (Thread, error) {
		...
		for projectIndex := range s.projects {
			...
			for threadIndex := range s.projects[projectIndex].Threads {
				...
				if thread.TitleLocked {
					// Automatic title generation is best-effort. Returning the existing
					// thread avoids surfacing an agent warning while preserving the lock.
					if autoGenerated {
						return cloneThread(*thread), nil
					}
					return Thread{}, ErrThreadTitleLocked
				}
				...
				if autoGenerated && thread.Worktree {
					nextBranch := namedWorktreeBranch(worktreeBranchPrefix(s.projects[projectIndex]), title, thread.ID)
...
func (s *Store) SetThreadTitleLocked(projectID, threadID string, locked bool) (Thread, error) {
	return withProjectMutationResult(s, func() (Thread, error) {
		for projectIndex := range s.projects {
			if s.projects[projectIndex].ID != projectID {
				continue
			...
			for threadIndex := range s.projects[projectIndex].Threads {
				...
				if thread.TitleLocked == locked {
					return cloneThread(*thread), nil
				}

				previous := thread.TitleLocked
...
func (s *Store) Add(name, path string, profileIDs ...string) (Project, error) {
...
func (s *Store) UpdateProject(projectID string, update ProjectUpdate) (result Project, err error) {
	if update.ProfileID == nil && update.WorktreeBranchPrefix == nil && update.RelatedProjects == nil &&
	...
	return withProjectMutationResult(s, func() (Project, error) {
		...
		return saveProjectMutationResult(s, s.projects[projectIndex], func() {
...
func (s *Store) ReorderProjects(profileID string, projectIDs []string) error {
	...
	return s.withProjectMutation(func() error {
		...
		return s.saveProjectMutation(func() {
			s.projects = previous
		})
	})
}
...
func (s *Store) ReorderThreads(projectID string, threadIDs []string) error {
	return s.withProjectMutation(func() error {
		for projectIndex := range s.projects {
			if s.projects[projectIndex].ID != projectID {
				continue
...
func (s *Store) UpdateThreadLimits(projectID, threadID string, tokenLimit *int64, costLimitUSD *float64) (Thread, error) {
...
func (s *Store) AddThreadWithOptions(projectID, title string, options AddThreadOptions) (Thread, error) {
	title = strings.TrimSpace(title)
	if title == "" {
...
func (s *Store) BeginThreadCreationRollback(projectID, threadID string) (bool, error) {
	return withProjectMutationResult(s, func() (bool, error) {
		for projectIndex := range s.projects {
...
func (s *Store) FinalizeThreadCreationRollback(projectID, threadID string) error {
	return s.withProjectMutation(func() error {
		for projectIndex := range s.projects {
			item := &s.projects[projectIndex]
			if item.ID != projectID {
...
func (s *Store) CommitThreadCreation(projectID, threadID string) (Thread, error) {
	return withProjectMutationResult(s, func() (Thread, error) {
		for projectIndex := range s.projects {
			item := &s.projects[projectIndex]
			if item.ID != projectID {
			...
			for threadIndex, thread := range item.Threads {
				...
				item.Threads[threadIndex].RollbackCleanupReady = false
				return saveProjectMutationResult(s, cloneThread(item.Threads[threadIndex]), func() {
					item.Threads[threadIndex] = thread
				})
			}
			return Thread{}, ErrThreadNotFound
...
func (s *Store) createWorktreeThread(item Project, thread Thread, baseBranch, baseRevision string, deferEnvironmentSetup bool) (Thread, error) {
	...
	thread.Branch = initialWorktreeBranch(item, thread)
...
func initialWorktreeBranch(item Project, thread Thread) string {
	prefix := worktreeBranchPrefix(item)
	if thread.Title == defaultThreadTitle {
		return prefix + "thread-" + shortThreadID(thread.ID)
	}
	return namedWorktreeBranch(prefix, thread.Title, thread.ID)
}

func namedWorktreeBranch(prefix, title, threadID string) string {
	return prefix + branchSlug(title) + "-" + shortThreadID(threadID)
}

func worktreeBranchPrefix(item Project) string {
	if item.WorktreeBranchPrefix == "" {
		return DefaultWorktreeBranchPrefix
	}
	return item.WorktreeBranchPrefix
}

func normalizeWorktreeBranchPrefix(value string) (string, error) {
	prefix := strings.TrimSpace(value)
	if prefix == "" {
		return "", errors.New("worktree branch prefix is required")
	}
	if utf8.RuneCountInString(prefix) > maxWorktreeBranchPrefixLength {
		return "", fmt.Errorf("worktree branch prefix must be %d characters or fewer", maxWorktreeBranchPrefixLength)
	}
	if !validGitBranchName(prefix + "thread-12345678") {
		return "", errors.New("worktree branch prefix must form a valid Git branch name")
	}
	return prefix, nil
}
