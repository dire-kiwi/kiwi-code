...
func (s *Store) SetThreadEnvironmentSetupStatus(projectID, threadID, status string) (result Thread, err error) {
	...
	for projectIndex := range s.projects {
		...
		for threadIndex := range s.projects[projectIndex].Threads {
			...
			if current == EnvironmentSetupSucceeded || current == EnvironmentSetupFailed {
				return cloneThread(*thread), nil
			}
			if current == status {
				return cloneThread(*thread), nil
