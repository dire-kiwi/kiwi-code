...
`scripts/worktree-setup.sh` prepares a worktree for isolated development launches: it seeds an isolated data directory (default `.kiwi-code-dev/data`) whose `settings.json` registers the user's personal Claude Code coding-agent profile ("CC Personal", `~/.claude-personal`), generates an isolated `kcv-*` tmux socket name, and writes both to `.kiwi-code-dev/env` for sourcing before `make dev`. It is idempotent and refuses the production data directory and socket. Use it when a development stack should offer the user's Claude Code profiles; seed the isolated data directory the same way (write `settings.json` with the `codingAgents` entry before first start) if a hand-rolled temporary data directory needs them. All other rules in this section still apply, including fresh ports, `--add-current-directory`, and cleaning up only the generated socket.
