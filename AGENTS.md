...
- tmux socket: `dire-mux` (used as `tmux -L dire-mux`)
- per-thread Shell session: `dire-mux-<projectID>-<threadID>-terminal`
- per-thread shared tools session: `dire-mux-<projectID>-<threadID>-tools`
...
The canonical `dire-mux` tmux server and port `4000` are reserved exclusively for production. Development mode must never use either one, and production started from a Git checkout must only run from `main`. Keep these startup checks fail-closed when changing launchers or runtime configuration.

## Isolate tests and validation

The canonical `dire-mux` tmux server is reserved exclusively for the user's production environment. Tests, browser validation, development, and agent-launched parallel stacks must never connect to it, inspect it, create sessions in it, or kill it.
